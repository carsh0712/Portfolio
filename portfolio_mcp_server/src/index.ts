...
const number = (description: string, defaults: JsonSchema = {}): JsonSchema => ({
  type: "number",
  description,
  ...defaults,
});
const boolean = (description: string, defaults: JsonSchema = {}): JsonSchema => ({
  type: "boolean",
  description,
  ...defaults,
});
...
const fileReferenceSchema: JsonSchema = {
  type: "object",
...
const screenshotSchema: JsonSchema = {
  type: "object",
  properties: {
...
async function callTool(name: string, args: ToolArgs): Promise<unknown> {
  switch (name) {
    ...
    case "portfolio_list_projects":
      return client.authJson(
        withQuery("/api/v1/projects/", args, ["portfolio_code", "page", "page_size", "search"])
      );
    case "portfolio_get_project":
      return client.authJson(
        `/api/v1/projects/${encodeURIComponent(requiredString(args, "portfolio_code"))}/${encodeURIComponent(
          requiredString(args, "project_code")
        )}`
      );
    ...
    case "portfolio_update_project":
      return client.authJson(
        ...
        { method: "PUT", body: requiredObject(args, "data") }
      );
    ...
    case "portfolio_get_public_projects":
      return client.publicJson(
        `/api/v1/public/${encodeURIComponent(requiredString(args, "username"))}/${encodeURIComponent(
          requiredString(args, "portfolio_code")
        )}/`
      );
    case "portfolio_get_public_project":
      return client.publicJson(
        `/api/v1/public/${encodeURIComponent(requiredString(args, "username"))}/${encodeURIComponent(
          requiredString(args, "portfolio_code")
        )}/${encodeURIComponent(requiredString(args, "project_code"))}/`
      );
...
function withQuery(endpoint: string, args: ToolArgs, keys: string[]): string {
...
function requireConfirm(args: ToolArgs, action: string): void {
  if (args.confirm !== true) {
    throw new Error(`Set confirm: true to ${action}.`);
  }
}

function omitConfirm(args: ToolArgs): ToolArgs {
...
function requiredObject(args: ToolArgs, key: string): Record<string, unknown> {
  const value = args[key];
