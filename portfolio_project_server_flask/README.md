...
포트폴리오 이미지와 프로젝트 스크린샷은 `jpg`, `jpeg`, `png`, `webp`만 허용합니다. `gif`는 리사이즈 및 WebP 변환 대상이 아니므로 업로드를 차단합니다.

- 최대 크기: `MAX_FILE_SIZE` 기본값 10 MB
- 상세 이미지: 긴 변 2048px 이하, WebP 품질 82
- 썸네일: 긴 변 480px 이하, WebP 품질 78
- 원본 보존: 이미지 원본 바이너리는 별도 보존하지 않고, 원본 파일명만 메타데이터로 보존

자세한 운영 기준은 `../docs/upload-policy.md`를 참고합니다.
